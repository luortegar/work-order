...
import { listClienUser } from 'src/api/userCrudApi';
...
export default function EmployeesList() {
  ...
  const columns: GridColDef[] = [
    {
        ...
        renderCell: (params: GridRenderCellParams) => (
          <Chip
            ...
            variant='outlined'
            size='small'
            onClick={() => navigate(`/home/client/${clientId}/employees/${params.row.userId}`)}
          />
        ),
    },
  ...
  const refreshData = (rowsPerPage:number, page:number, sort:string, filterName:string) => {
    if(clientId){
      listClienUser(clientId, rowsPerPage, page, sort, filterName).then((r) => {
